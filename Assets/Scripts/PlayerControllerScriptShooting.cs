...
public class PlayerControllerScriptShooting : MonoBehaviour {
    ...
    public void Shoot() {
        GameObject instantiatedBullet = Instantiate(bulletPrefab, bulletSource.transform.position, bulletSource.transform.rotation) as GameObject;
        if (facingRight)
        {
            instantiatedBullet.GetComponent<Rigidbody2D>().velocity = new Vector2(bulletSpeedX, bulletSpeedY);
        }
        else
        {
            instantiatedBullet.GetComponent<Rigidbody2D>().velocity = new Vector2(bulletSpeedX * -1, bulletSpeedY);
        }
    }
